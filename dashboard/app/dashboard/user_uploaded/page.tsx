...
export default async function UserUploadedPage() {
  ...
  // Determine the most common classification for header styling
  const classifications = articles.map(a => a.classification).filter(c => c && c !== '')
  const mostCommon = classifications.length > 0 ? classifications[0] : 'Neutral'
              ...
              <div className="px-4 lg:px-6">
                {articles.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-12 text-center border rounded-lg">
                    <p className="text-muted-foreground text-lg">No uploaded articles found</p>
                    <p className="text-muted-foreground text-sm mt-2">
                      Upload your first article from the dashboard
                    </p>
                  </div>
                ) : (
                  <FilteredArticlesTable articles={articles} classification={mostCommon as any} />
                )}
              </div>
            </div>
          </div>
